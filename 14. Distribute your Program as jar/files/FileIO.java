...
public class FileIO{
	public static void loadFromFile(Inventory inventory){
		try{
			...
			File fileElectornics = new File(folder.getName()+"/electronics.txt");
			
			if(!fileElectornics.exists()){
				fileElectornics.createNewFile();
			}
			...
			File fileClothing = new File(folder.getName()+"/clothing.txt");
			if(!fileClothing.exists()){
				fileClothing.createNewFile();
			}
